...
- the title-screen pi extension: same treatment
  (`scripts/test-title-screen.mjs`) — art geometry (flush left, width
  independent), the random role pick (pool membership + rng mapping),
  caption contents, and startup gating against a fake pi — plus the
  orphan-hex scan now covers both pi extensions (no hardcoded colors)
