...
- the title-screen pi extension: same treatment
  (`scripts/test-title-screen.mjs`) — art geometry, the random role pick
  (pool membership + rng mapping), caption contents and centering, and
  startup gating against a fake pi — plus the orphan-hex scan now covers
  both pi extensions (no hardcoded colors)
